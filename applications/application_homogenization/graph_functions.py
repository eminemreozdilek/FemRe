...
class FunctionsGraph(object):
    ...
    def show_voxel_mesh(self):
        ...
        self.__plotter.add_mesh(grid_mesh, show_edges=True, color="cyan")
    ...
    def show_strut_mesh(self):
        ...
        self.__plotter.add_mesh(strut_lines, line_width=10)

        if self.__flag_show_nodes:
            mesh_plotter.plot_nodal_points(self.__plotter, nodes, "ID", model_size=model_geometry_size)
        if self.__flag_show_edges_labels:
            mesh_plotter.plot_strut_labels(self.__plotter, struts, model_size=model_geometry_size)
